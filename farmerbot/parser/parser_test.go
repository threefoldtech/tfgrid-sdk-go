...
func TestJsonParsers(t *testing.T) {
	...
	t.Run("test invalid json no node ID", func(t *testing.T) {
		farmContent := `{ "ID": 1 }`
		nodeContent := `{ "twin_id" : 1, "resources": { "total": { "SRU": 1, "CRU": 1, "HRU": 1, "MRU": 1 } } }`
		content := fmt.Sprintf(`
		{ 
			"nodes": [ %v ],
			"farm": %v, 
	...
	t.Run("test invalid json no node sru", func(t *testing.T) {
		farmContent := `{ "ID": 1 }`
		nodeContent := `{ "ID": 1, "twin_id" : 1, "resources": { "total": { "CRU": 1, "HRU": 1, "MRU": 1 } } }`
		content := fmt.Sprintf(`
		{ 
			"nodes": [ %v ],
			"farm": %v, 
	...
	t.Run("test invalid json no cru", func(t *testing.T) {
		farmContent := `{ "ID": 1 }`
		nodeContent := `{ "ID": 1, "twin_id" : 1, "resources": { "total": { "SRU": 1, "HRU": 1, "MRU": 1 } } }`
		content := fmt.Sprintf(`
		{ 
			"nodes": [ %v ],
			"farm": %v, 
	...
	t.Run("test invalid json no mru", func(t *testing.T) {
		farmContent := `{ "ID": 1 }`
		nodeContent := `{ "ID": 1, "twin_id" : 1, "resources": { "total": { "SRU": 1, "CRU": 1, "HRU": 1 } } }`
		content := fmt.Sprintf(`
		{ 
			"nodes": [ %v ],
			"farm": %v, 
	...
	t.Run("test invalid json node over provision CPU", func(t *testing.T) {
		farmContent := `{ "ID": 1 }`
		nodeContent := `{ "ID": 1, "twin_id" : 1, "resources": { "overprovision_cpu": 5, "total": { "SRU": 1, "CRU": 1, "HRU": 1, "CRU": 1 } } }`
		content := fmt.Sprintf(`
		{ 
			"nodes": [ %v ],
			"farm": %v, 
