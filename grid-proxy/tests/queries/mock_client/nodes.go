		...
		if node.satisfies(filter, &g.data) {
			...
			res = append(res, types.Node{
				...
				Speed: types.Speed{
					...
					UDPDownloadIPv4: g.data.Speeds[uint32(node.TwinID)].UDPDownloadIPv4,
					UDPUploadIPv4:   g.data.Speeds[uint32(node.TwinID)].UDPUploadIPv4,
					TCPDownloadIPv6: g.data.Speeds[uint32(node.TwinID)].TCPDownloadIPv6,
					TCPUploadIPv6:   g.data.Speeds[uint32(node.TwinID)].TCPUploadIPv6,
					UDPDownloadIPv6: g.data.Speeds[uint32(node.TwinID)].UDPDownloadIPv6,
					UDPUploadIPv6:   g.data.Speeds[uint32(node.TwinID)].UDPUploadIPv6,
				},
				CpuBenchmark: types.CpuBenchmark{
					SingleThreaded: g.data.CpuBenchmarks[uint32(node.TwinID)].SingleThreaded,
					MultiThreaded:  g.data.CpuBenchmarks[uint32(node.TwinID)].MultiThreaded,
					Threads:        g.data.CpuBenchmarks[uint32(node.TwinID)].Threads,
					Workloads:      g.data.CpuBenchmarks[uint32(node.TwinID)].Workloads,
				},
