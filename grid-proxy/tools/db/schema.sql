...
SELECT node.node_id,
    ...
    (
        COALESCE(sum(contract_resources.mru), (0)::numeric) + (
            GREATEST(
                ((node_resources_total.mru / (10)::numeric))::bigint,
                '2147483648'::bigint
            )
        )::numeric
    ) AS used_mru,
    ...
    (
        COALESCE(sum(contract_resources.sru), (0)::numeric) + ('107374182400'::bigint)::numeric
    ) AS used_sru,
    (
        (
            node_resources_total.mru - COALESCE(sum(contract_resources.mru), (0)::numeric)
        ) - (
            GREATEST(
                ((node_resources_total.mru / (10)::numeric))::bigint,
                '2147483648'::bigint
    ...
    (
        node_resources_total.hru - COALESCE(sum(contract_resources.hru), (0)::numeric)
    ) AS free_hru,
    (
        (
            node_resources_total.sru - COALESCE(sum(contract_resources.sru), (0)::numeric)
        ) - ('107374182400'::bigint)::numeric
    ) AS free_sru,
...
FROM (
        (
            (
                public.contract_resources
                JOIN public.node_contract node_contract ON (
                    (
                        (
                            (node_contract.resources_used_id)::text = (contract_resources.id)::text
                        )
                        AND (
                            (node_contract.state)::text = ANY (
                                (
                                    ARRAY ['Created'::character varying, 'GracePeriod'::character varying]
                                )::text []
                            )
                        )
                    )
        ...
        JOIN public.node_resources_total node_resources_total ON (
            (
                (node_resources_total.node_id)::text = (node.id)::text
            )
        )
    )
