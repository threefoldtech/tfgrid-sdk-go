...
-- ============================================================================
-- Tests for node INSERT and DELETE operations on nodex
...
-- Test 1: INSERT node should populate nodex
...
SELECT ok(
    EXISTS (
        SELECT 1 FROM nodex WHERE node_id = 1001
    ),
    'Node INSERT should create entry in nodex'
);

-- Test 2: Verify nodex values match expected from view
SELECT is(
    (SELECT total_hru FROM nodex WHERE node_id = 1001),
    1000000000000,
    'nodex total_hru should match node_resources_total'
);

SELECT is(
    (SELECT total_mru FROM nodex WHERE node_id = 1001),
    100000000000,
    'nodex total_mru should match node_resources_total'
);

-- Test 3: DELETE node should remove entry from nodex
...
SELECT ok(
    ...
    'Node DELETE should remove entry from nodex'
);
...
SELECT ok(
    ...
    'Node INSERT should populate all fields in nodex'
);
