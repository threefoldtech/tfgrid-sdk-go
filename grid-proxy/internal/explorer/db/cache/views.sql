...
SELECT
    ...
    COALESCE(node_gpu_agg.free_gpu_count, 0) as free_gpu_count,
    NOW() as updated_at
FROM node
    ...
    LEFT JOIN(
        SELECT
            ...
            COUNT(g1.id) FILTER (WHERE g1.contract IS NULL OR g1.contract = 0) as free_gpu_count,
...
GROUP BY
    ...
    COALESCE(node_gpu_agg.free_gpu_count, 0),
    NOW(),
    node.country,
    COALESCE(dmi.bios, '{}'),
