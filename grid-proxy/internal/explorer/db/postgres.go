...
var views = []string{
	...
	CREATE OR REPLACE FUNCTION refresh_node_materialized_view()
		RETURNS TRIGGER AS $$
	BEGIN
		REFRESH MATERIALIZED VIEW node_materialized_view;
		RETURN NULL;
	END;
	$$ LANGUAGE plpgsql;
	...
	FOR EACH ROW
	EXECUTE FUNCTION refresh_node_materialized_view();
